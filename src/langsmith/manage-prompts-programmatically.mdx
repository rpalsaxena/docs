...
The LangSmith SDK includes built-in caching for prompts. When enabled, pulled prompts are cached locally, reducing latency and API calls for frequently used prompts.

**Requirements:**
- Python SDK: `langsmith >= 0.5.2`
...
# Check cache metrics
print(f"Cache hits: {client._prompt_cache.metrics.hits}")
print(f"Cache misses: {client._prompt_cache.metrics.misses}")
print(f"Hit rate: {client._prompt_cache.metrics.hit_rate:.1%}")
```
...
// Check cache metrics
console.log(`Cache hits: ${client.promptCache?.metrics.hits}`);
console.log(`Cache misses: ${client.promptCache?.metrics.misses}`);
console.log(`Hit rate: ${(client.promptCache?.hitRate ?? 0 * 100).toFixed(1)}%`);
```

</CodeGroup>

You can also enable caching via environment variables:

```bash
export LANGSMITH_PROMPT_CACHE_ENABLED=true
export LANGSMITH_PROMPT_CACHE_MAX_SIZE=100
export LANGSMITH_PROMPT_CACHE_TTL_SECONDS=3600
```
...
client = Client(prompt_cache_enabled=True)
...
client._prompt_cache.dump("prompts_cache.json")
client.cleanup()
```
...
// Create client with caching enabled
const client = new Client({ promptCacheEnabled: true });
...
// Export cache to a file
client.promptCache?.dump("prompts_cache.json");
client.cleanup();
```
...
# Load cached prompts with infinite TTL (never expire)
client = Client(
    prompt_cache_enabled=True,
    prompt_cache_ttl_seconds=None,  # Infinite TTL
    prompt_cache_path="prompts_cache.json",
)
...
const client = new Client({
  promptCacheEnabled: true,
  promptCacheTtlSeconds: null,  // Infinite TTL
  promptCachePath: "prompts_cache.json",
});
...
```bash
export LANGSMITH_PROMPT_CACHE_ENABLED=true
export LANGSMITH_PROMPT_CACHE_TTL_SECONDS=  # Empty = infinite TTL
export LANGSMITH_PROMPT_CACHE_PATH=/path/to/prompts_cache.json
```
