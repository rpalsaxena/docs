...
For production deployments with key rotation and audit logging, see [Envelope encryption with AWS KMS](#envelope-encryption-with-aws-kms).
...
async def encrypt_json(ctx: EncryptionContext, data: dict) -> dict:
    ...
    for key, value in data.items():
        if key in SENSITIVE_FIELDS and value is not None:
            result[key] = fernet.encrypt(str(value).encode()).decode()
        else:
            result[key] = value
    return result
...
async def encrypt_json(ctx: EncryptionContext, data: dict) -> dict:
    ...
    for key, value in data.items():
        if key in SENSITIVE_FIELDS and value is not None:
            ciphertext, _ = client.encrypt(
                source=str(value).encode(),
                materials_manager=cmm,
                encryption_context={"tenant_id": ctx.metadata["tenant_id"]},
            )
            result[key] = base64.b64encode(ciphertext).decode()
        else:
            result[key] = value
    return result
