...
3. Agent also loads 2-3 relevant example queries:
...
- **Semantic search**: Embed example queries and retrieve the most similar ones based on user query
- **Tag-based lookup**: Tag examples by pattern (joins, aggregations, date filters) and load relevant tags
- **Hybrid**: Load schema first, then search examples within that skill's domain

**Benefits:**
- **Better query quality**: Examples demonstrate patterns, idioms, and edge cases
- **Consistency**: Examples enforce coding standards and best practices
- **Reduced hallucination**: Concrete examples ground the agent's responses

**Trade-offs:**
- Additional latency from example retrieval
- More complex implementation (requires example storage and search)
- Context window usage increases with examples
