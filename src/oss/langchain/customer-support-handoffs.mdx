...
The [handoffs pattern](/oss/langchain/multi-agent#handoffs) describes workflows where an agent's behavior changes as it moves through different steps of a task. This tutorial shows how to implement handoffs by using tool calls to dynamically change a single agent's configuration—updating its available tools and instructions at each step based on what happened earlier in the conversation.
...
<Tip>
If your workflow requires arbitrary transitions between most steps, consider whether you need a structured workflow at all. The handoffs pattern works best when steps follow a clear sequential progression with occasional backwards transitions for corrections.
</Tip>
...
This example demonstrates the handoffs pattern.
