...
- One of an OpenAI or Anthropic API key: Required for Agent Builder to make API calls to LLMs. The agent graphs load this key from workspace secrets for inference.

<Note icon="wand-magic-sparkles" iconType="regular">
  Agent Builder supports custom models. You can override the default Anthropic model by providing a different `modelId` and a matching secret name via `envVarName` (see “Custom models” for examples).
</Note>
